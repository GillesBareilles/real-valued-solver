...
private:
  ...
  Eigen::SimplicialLDLT<SpMat > m_solverBAlpha;
...
public:
  ...
  RealValuedSolverLDLT(const SpMat& R, const SpMat& S, double CG_tol, const double alpha);
  ~RealValuedSolverLDLT () {};
...
RealValuedSolverLDLT::RealValuedSolverLDLT(const SpMat& R, const SpMat& S, double CG_tol = 1e-16, const double alpha = 1) : m_R(R), m_S(S), m_CG_tol(CG_tol), m_alpha(alpha), m_solverBAlpha()
{
  m_adjustAlpha = false;
  m_CG_maxit = 50;
...
void RealValuedSolverLDLT::setTolerance(const double tol)
{
  m_CG_tol = tol;
}

void RealValuedSolverLDLT::setMaxIt(const int maxIt)
{
  m_CG_maxit = maxIt;
}

void RealValuedSolverLDLT::factorize()
{
...
void RealValuedSolverLDLT::compute()
{
  factorize();
}

Eigen::MatrixXcd RealValuedSolverLDLT::solve(const Eigen::MatrixXcd& rhs)
{
...
void RealValuedSolverLDLT::info()
{
...
void RealValuedSolverLDLT::conjugateGradient(const Eigen::VectorXd& rhs, Eigen::VectorXd& x)
{
  using std::sqrt;
  using std::abs;
...
Eigen::Matrix<double, 10, 1> RealValuedSolverLDLT::information() const
{
